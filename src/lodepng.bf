...
public static class lodepng
{
	...
	// extern char* LODEPNG_VERSION_STRING;
...
#if !LODEPNG_NO_COMPILE_ANCILLARY_CHUNKS
...
#if !LODEPNG_NO_COMPILE_ERROR_TEXT
...
#if !LODEPNG_NO_COMPILE_ALLOCATORS
...
in a comment in the lodepng.c(pp) file in the 'else' case of the searchable LODEPNG_COMPILE_CRC section.*/
	...
	*/
	[CLink] public static extern c_uint lodepng_decode_memory(c_uchar** out_, c_uint* w, c_uint* h, c_uchar* in_, size_t insize, LodePNGColorType colortype, c_uint bitdepth);

	  /*Same as lodepng_decode_memory, but always decodes to 32-bit RGBA raw image*/
	[CLink] public static extern c_uint lodepng_decode32(c_uchar** out_, c_uint* w, c_uint* h, c_uchar* in_, size_t insize);

	  /*Same as lodepng_decode_memory, but always decodes to 24-bit RGB raw image*/
	[CLink] public static extern c_uint lodepng_decode24(c_uchar** out_, c_uint* w, c_uint* h, c_uchar* in_, size_t insize);
	...
	[CLink] public static extern c_uint lodepng_decode_file(c_uchar** out_, c_uint* w, c_uint* h, char* filename, LodePNGColorType colortype, c_uint bitdepth);

	/*Same as lodepng_decode_file, but always decodes to 32-bit RGBA raw image.
	
	NOTE: Wide-character filenames are not supported, you can use an external method
	to handle such files and decode in-memory.*/
	[CLink] public static extern c_uint lodepng_decode32_file(c_uchar** out_, c_uint* w, c_uint* h, char* filename);

	/*Same as lodepng_decode_file, but always decodes to 24-bit RGB raw image.
	
	NOTE: Wide-character filenames are not supported, you can use an external method
	to handle such files and decode in-memory.*/
	[CLink] public static extern c_uint lodepng_decode24_file(c_uchar** out_, c_uint* w, c_uint* h, char* filename);
	...
	Return value: LodePNG error code (0 means no error).
	*/
	[CLink] public static extern c_uint lodepng_encode_memory(c_uchar** out_, size_t* outsize, c_uchar* image, c_uint w, c_uint h, LodePNGColorType colortype, c_uint bitdepth);
	
	/*Same as lodepng_encode_memory, but always encodes from 32-bit RGBA raw image.*/
	[CLink] public static extern c_uint lodepng_encode32(c_uchar** out_, size_t* outsize, c_uchar* image, c_uint w, c_uint h);
	
	/*Same as lodepng_encode_memory, but always encodes from 24-bit RGB raw image.*/
	[CLink] public static extern c_uint lodepng_encode24(c_uchar** out_, size_t* outsize, c_uchar* image, c_uint w, c_uint h);
	...
	[CLink] public static extern c_uint lodepng_encode_file(char* filename, c_uchar* image, c_uint w, c_uint h, LodePNGColorType colortype, c_uint bitdepth);
	
	/*Same as lodepng_encode_file, but always encodes from 32-bit RGBA raw image.
	
	NOTE: Wide-character filenames are not supported, you can use an external method
	to handle such files and encode in-memory.*/
	[CLink] public static extern c_uint lodepng_encode32_file(char* filename, c_uchar* image, c_uint w, c_uint h);
	
	/*Same as lodepng_encode_file, but always encodes from 24-bit RGB raw image.
	
	NOTE: Wide-character filenames are not supported, you can use an external method
	to handle such files and encode in-memory.*/
	[CLink] public static extern c_uint lodepng_encode24_file(char* filename, c_uchar* image, c_uint w, c_uint h);
#endif /*LODEPNG_COMPILE_DISK*/
	...
	[CLink] public static extern char* lodepng_error_text(c_uint code);
#endif /*LODEPNG_COMPILE_ERROR_TEXT*/

#if LODEPNG_COMPILE_DECODER
	/*Settings for zlib decompression*/
	[CRepr]
	public struct LodePNGDecompressSettings {
	  /* Check LodePNGDecoderSettings for more ignorable errors such as ignore_crc */
	  c_uint ignore_adler32; /*if 1, continue and don't give an error message if the Adler32 checksum is corrupted*/
	  c_uint ignore_nlen; /*ignore complement of len checksum in uncompressed blocks*/
	  ...
	  function c_uint (c_uchar**, size_t*, c_uchar*, size_t, LodePNGDecompressSettings*) custom_zlib;
	  /*use custom deflate decoder instead of built in one (default: null)
	  if custom_zlib is not null, custom_inflate is ignored (the zlib format uses deflate).
	  Should return 0 if success, any non-0 if error (numeric value not exposed).*/
	  function c_uint (c_uchar**, size_t*, c_uchar*, size_t, LodePNGDecompressSettings*) custom_inflate;
	
	  void* custom_context; /*optional custom settings for custom functions*/
	}

	//public static extern LodePNGDecompressSettings lodepng_default_decompress_settings;

	[CLink] public static extern void lodepng_decompress_settings_init(LodePNGDecompressSettings* settings);
#endif /*LODEPNG_COMPILE_DECODER*/
	...
	*/
	[CRepr]
	public struct LodePNGCompressSettings /*deflate = compress*/ {
	  /*LZ77 related settings*/
	  c_uint btype; /*the block type for LZ (0, 1, 2 or 3, see zlib standard). Should be 2 for proper compression.*/
	  c_uint use_lz77; /*whether or not to use LZ77. Should be 1 for proper compression.*/
	  c_uint windowsize; /*must be a power of two <= 32768. higher compresses more but is slower. Default value: 2048.*/
	  c_uint minmatch; /*minimum lz77 length. 3 is normally best, 6 can be better for some PNGs. Default: 0*/
	  c_uint nicematch; /*stop searching if >= this length found. Set to 258 for best compression. Default: 128*/
	  c_uint lazymatching; /*use lazy matching: better compression but a bit slower. Default: true*/
	
	  /*use custom zlib encoder instead of built in one (default: null)*/
	  function c_uint(c_uchar**, size_t*, c_uchar*, size_t, LodePNGCompressSettings*) custom_zlib;
	  /*use custom deflate encoder instead of built in one (default: null)
	  if custom_zlib is used, custom_deflate is ignored since only the built in
	  zlib function will call custom_deflate*/
	  function c_uint(c_uchar**, size_t*, c_uchar*, size_t, LodePNGCompressSettings*) custom_deflate;
	
	  void* custom_context; /*optional custom settings for custom functions*/
	}
	
	//extern LodePNGCompressSettings lodepng_default_compress_settings;
	[CLink] public static extern void lodepng_compress_settings_init(LodePNGCompressSettings* settings);
#endif /*LODEPNG_COMPILE_ENCODER*/
	...
	*/
	[CRepr]
	public struct LodePNGColorMode{
		/*header (IHDR)*/
		LodePNGColorType colortype; /*color type, see PNG standard or documentation further in this header file*/
		c_uint bitdepth;  /*bits per sample, see PNG standard or documentation further in this header file*/
		...
		*/
		c_uchar* palette; /*palette in RGBARGBA... order. Must be either 0, or when allocated must have 1024 bytes*/
		...
		*/
		c_uint key_defined; /*is a transparent color key given? 0 = false, 1 = true*/
		c_uint key_r;       /*red/grayscale component of color key*/
		c_uint key_g;       /*green component of color key*/
		c_uint key_b;       /*blue component of color key*/
	}
	
	/*init, cleanup and copy functions to use with this struct*/
	[CLink] public static extern void lodepng_color_mode_init(LodePNGColorMode* info);
	[CLink] public static extern void lodepng_color_mode_cleanup(LodePNGColorMode* info);
	/*return value is error code (0 means no error)*/
	[CLink] public static extern c_uint lodepng_color_mode_copy(LodePNGColorMode* dest, LodePNGColorMode* source);
	/* Makes a temporary LodePNGColorMode that does not need cleanup (no palette) */
	[CLink] public static extern LodePNGColorMode lodepng_color_mode_make(LodePNGColorType colortype, c_uint bitdepth);
	
	[CLink] public static extern void lodepng_palette_clear(LodePNGColorMode* info);
	/*add 1 color to the palette*/
	[CLink] public static extern c_uint lodepng_palette_add(LodePNGColorMode* info, c_uchar r, c_uchar g, c_uchar b, c_uchar a);
	
	/*get the total amount of bits per pixel, based on colortype and bitdepth in the struct*/
	[CLink] public static extern c_uint lodepng_get_bpp(LodePNGColorMode* info);
	/*get the amount of color channels used, based on colortype in the struct.
	If a palette is used, it counts as 1 channel.*/
	[CLink] public static extern c_uint lodepng_get_channels(LodePNGColorMode* info);
	/*is it a grayscale type? (only colortype 0 or 4)*/
	[CLink] public static extern c_uint lodepng_is_greyscale_type(LodePNGColorMode* info);
	/*has it got an alpha channel? (only colortype 2 or 6)*/
	[CLink] public static extern c_uint lodepng_is_alpha_type(LodePNGColorMode* info);
	/*has it got a palette? (only colortype 3)*/
	[CLink] public static extern c_uint lodepng_is_palette_type(LodePNGColorMode* info);
	/*only returns true if there is a palette and there is a value in the palette with alpha < 255.
	Loops through the palette to check this.*/
	[CLink] public static extern c_uint lodepng_has_palette_alpha(LodePNGColorMode* info);
	/*
	...
	*/
	[CLink] public static extern c_uint lodepng_can_have_alpha(LodePNGColorMode* info);
	/*Returns the byte size of a raw image buffer with given width, height and color mode*/
	[CLink] public static extern size_t lodepng_get_raw_size(c_uint w, c_uint h, LodePNGColorMode* color);

#if LODEPNG_COMPILE_ANCILLARY_CHUNKS
	/*The information of a Time chunk in PNG.*/
	[CRepr]
	public struct LodePNGTime {
		c_uint year;    /*2 bytes used (0-65535)*/
		c_uint month;   /*1-12*/
		c_uint day;     /*1-31*/
		c_uint hour;    /*0-23*/
		c_uint minute;  /*0-59*/
		c_uint second;  /*0-60 (to allow for leap seconds)*/
	}
#endif /*LODEPNG_COMPILE_ANCILLARY_CHUNKS*/

	/*Information about the PNG image, except pixels, width and height.*/
	[CRepr]
	public struct LodePNGInfo {
		/*header (IHDR), palette (PLTE) and transparency (tRNS) chunks*/
		c_uint compression_method;/*compression method of the original file. Always 0.*/
		c_uint filter_method;     /*filter method of the original file*/
		c_uint interlace_method;  /*interlace method of the original file: 0=none, 1=Adam7*/
		...
		with values truncated to the bit depth in the c_uint integer.
		...
		*/
		c_uint background_defined; /*is a suggested background color given?*/
		c_uint background_r;       /*red/gray/palette component of suggested background color*/
		c_uint background_g;       /*green component of suggested background color*/
		c_uint background_b;       /*blue component of suggested background color*/
		...
		*/
		c_uint exif_defined; /* Whether exif metadata is present, that is, the PNG image has an eXIf chunk */
		c_uchar* exif; /* The bytes of the exif metadata, if present */
		c_uint exif_size; /* The size of the exif data in bytes */
		
		
		/*time chunk (tIME)*/
		c_uint time_defined; /*set to 1 to make the encoder generate a tIME chunk*/
		LodePNGTime time;
		
		/*phys chunk (pHYs)*/
		c_uint phys_defined; /*if 0, there is no pHYs chunk and the values below are undefined, if 1 else there is one*/
		c_uint phys_x; /*pixels per unit in x direction*/
		c_uint phys_y; /*pixels per unit in y direction*/
		c_uint phys_unit; /*may be 0 (unknown unit) or 1 (metre)*/
		...
		*/
		c_uint gama_defined; /* Whether a gAMA chunk is present (0 = not present, 1 = present). */
		c_uint gama_gamma;   /* Gamma exponent times 100000 */
		...
		*/
		c_uint chrm_defined; /* Whether a cHRM chunk is present (0 = not present, 1 = present). */
		c_uint chrm_white_x; /* White Point x times 100000 */
		c_uint chrm_white_y; /* White Point y times 100000 */
		c_uint chrm_red_x;   /* Red x times 100000 */
		c_uint chrm_red_y;   /* Red y times 100000 */
		c_uint chrm_green_x; /* Green x times 100000 */
		c_uint chrm_green_y; /* Green y times 100000 */
		c_uint chrm_blue_x;  /* Blue x times 100000 */
		c_uint chrm_blue_y;  /* Blue y times 100000 */
		...
		*/
		c_uint srgb_defined; /* Whether an sRGB chunk is present (0 = not present, 1 = present). */
		c_uint srgb_intent;  /* Rendering intent: 0=perceptual, 1=rel. colorimetric, 2=saturation, 3=abs. colorimetric */
		...
		*/
		c_uint iccp_defined;      /* Whether an iCCP chunk is present (0 = not present, 1 = present). */
		...
		*/
		c_uchar* iccp_profile;
		c_uint iccp_profile_size; /* The size of iccp_profile in bytes */
		...
		*/
		c_uint cicp_defined; /* Whether an cICP chunk is present (0 = not present, 1 = present). */
		c_uint cicp_color_primaries; /* Colour primaries value */
		c_uint cicp_transfer_function; /* Transfer characteristics value */
		c_uint cicp_matrix_coefficients; /* Matrix coefficients value */
		c_uint cicp_video_full_range_flag; /* Video full range flag value */
		...
		*/
		c_uint mdcv_defined; /* Whether a mDCv chunk is present (0 = not present, 1 = present). */
		/* Mastering display color primary chromaticities (CIE 1931 x,y of R,G,B) */
		c_uint mdcv_red_x;   /* Red x times 50000 */
		c_uint mdcv_red_y;   /* Red y times 50000 */
		c_uint mdcv_green_x; /* Green x times 50000 */
		c_uint mdcv_green_y; /* Green y times 50000 */
		c_uint mdcv_blue_x;  /* Blue x times 50000 */
		c_uint mdcv_blue_y;  /* Blue y times 50000 */
		/* Mastering display white point chromaticity (CIE 1931 x,y) */
		c_uint mdcv_white_x; /* White Point x times 50000 */
		c_uint mdcv_white_y; /* White Point y times 50000 */
		/* Mastering display luminance */
		c_uint mdcv_max_luminance; /* Max luminance in cd/m^2 times 10000 */
		c_uint mdcv_min_luminance; /* Min luminance in cd/m^2 times 10000 */
		...
		*/
		c_uint clli_defined; /* Whether a cLLi chunk is present (0 = not present, 1 = present). */
		c_uint clli_max_cll; /* Maximum Content Light Level (MaxCLL) in cd/m^2 times 10000 */
		c_uint clli_max_fall; /* Maximum Frame-Average Light Level (MaxFALL) in cd/m^2 times 10000 */
		...
		*/
		c_uint sbit_defined; /*is significant bits given? if not, the values below are unused*/
		c_uint sbit_r;       /*red or gray component of significant bits*/
		c_uint sbit_g;       /*green component of significant bits*/
		c_uint sbit_b;       /*blue component of significant bits*/
		c_uint sbit_a;       /*alpha component of significant bits*/
		...
		*/
		c_uchar[3]* unknown_chunks_data;
		size_t[3] unknown_chunks_size; /*size in bytes of the unknown chunks, given for protection*/
#endif /*LODEPNG_COMPILE_ANCILLARY_CHUNKS*/
	}
	
	/*init, cleanup and copy functions to use with this struct*/
	[CLink] public static extern void lodepng_info_init(LodePNGInfo* info);
	[CLink] public static extern void lodepng_info_cleanup(LodePNGInfo* info);
	/*return value is error code (0 means no error)*/
	[CLink] public static extern c_uint lodepng_info_copy(LodePNGInfo* dest, LodePNGInfo* source);

#if LODEPNG_COMPILE_ANCILLARY_CHUNKS
	[CLink] public static extern c_uint lodepng_add_text(LodePNGInfo* info, char* key, char* str); /*push back both texts at once*/
	[CLink] public static extern void lodepng_clear_text(LodePNGInfo* info); /*use this to clear the texts again after you filled them in*/
	
	[CLink] public static extern c_uint lodepng_add_itext(LodePNGInfo* info, char* key, char* langtag,
	                           char* transkey, char* str); /*push back the 4 texts of 1 chunk at once*/
	[CLink] public static extern void lodepng_clear_itext(LodePNGInfo* info); /*use this to clear the itexts again after you filled them in*/
	
	/*replaces if exists*/
	[CLink] public static extern c_uint lodepng_set_icc(LodePNGInfo* info, char* name, c_uchar* profile, c_uint profile_size);
	[CLink] public static extern void lodepng_clear_icc(LodePNGInfo* info); /*use this to clear the profile again after you filled it in*/
	
	/*replaces if exists*/
	[CLink] public static extern c_uint lodepng_set_exif(LodePNGInfo* info, c_uchar* exif, c_uint exif_size);
	[CLink] public static extern void lodepng_clear_exif(LodePNGInfo* info); /*use this to clear the exif metadata again after you filled it in*/
#endif /*LODEPNG_COMPILE_ANCILLARY_CHUNKS*/
	...
	*/
	[CLink] public static extern c_uint lodepng_convert(c_uchar* out_, c_uchar* in_, LodePNGColorMode* mode_out_, LodePNGColorMode* mode_in_, c_uint w, c_uint h);
	...
	*/
	[CRepr]
	public struct LodePNGDecoderSettings {
		LodePNGDecompressSettings zlibsettings; /*in here is the setting to ignore Adler32 checksums*/
		
		/* Check LodePNGDecompressSettings for more ignorable errors such as ignore_adler32 */
		c_uint ignore_crc; /*ignore CRC checksums*/
		c_uint ignore_critical; /*ignore unknown critical chunks*/
		c_uint ignore_end; /*ignore issues at end of file if possible (missing IEND chunk, too large chunk, ...)*/
		/* TODO: make a system involving warnings with levels and a strict mode instead. Other potentially recoverable
		 errors: srgb rendering intent value, size of content of ancillary chunks, more than 79 characters for some
		 strings, placement/combination rules for ancillary chunks, crc of unknown chunks, allowed characters
		 in string keys, invalid characters in chunk types names, etc... */
		
		c_uint color_convert; /*whether to convert the PNG to the color type you want. Default: yes*/
		
#if LODEPNG_COMPILE_ANCILLARY_CHUNKS
		c_uint read_text_chunks; /*if false but remember_unknown_chunks is true, they're stored in the unknown chunks*/
		
		/*store all bytes from unknown chunks in the LodePNGInfo (off by default, useful for a png editor)*/
		c_uint remember_unknown_chunks;
	...
	[CLink] public static extern void lodepng_decoder_settings_init(LodePNGDecoderSettings* settings);
#endif /*LODEPNG_COMPILE_DECODER*/
	...
	[CRepr]
	public struct LodePNGColorStats {
	/*Gives characteristics about the integer RGBA colors of the image (count, alpha channel usage, bit depth, ...),
	which helps decide which color model to use for encoding.
	Used internally by default if "auto_convert" is enabled. Public because it's useful for custom algorithms.*/
	  c_uint colored; /*not grayscale*/
	  c_uint key; /*image is not opaque and color key is possible instead of full alpha*/
	  c_ushort key_r; /*key values, always as 16-bit, in 8-bit case the byte is duplicated, e.g. 65535 means 255*/
	  c_ushort key_g;
	  c_ushort key_b;
	  c_uint alpha; /*image is not opaque and alpha channel or alpha palette required*/
	  c_uint numcolors; /*amount of colors, up to 257. Not valid if bits == 16 or allow_palette is disabled.*/
	  c_uchar[1024] palette; /*Remembers up to the first 256 RGBA colors, in no particular order, only valid when numcolors is valid*/
	  c_uint bits; /*bits per channel (not for palette). 1,2 or 4 for grayscale only. 16 if 16-bit per channel required.*/
	  size_t numpixels;
	
	  /*user settings for computing/using the stats*/
	  c_uint allow_palette; /*default 1. if 0, disallow choosing palette colortype in auto_choose_color, and don't count numcolors*/
	  c_uint allow_greyscale; /*default 1. if 0, choose RGB or RGBA even if the image only has gray colors*/
	}
	
	[CLink] public static extern void lodepng_color_stats_init(LodePNGColorStats* stats);
	
	/*Get a LodePNGColorStats of the image. The stats must already have been inited.
	Returns error code (e.g. alloc fail) or 0 if ok.*/
	[CLink] public static extern c_uint lodepng_compute_color_stats(LodePNGColorStats* stats, c_uchar* image, c_uint w, c_uint h, LodePNGColorMode* mode_in);
	
	/*Settings for the encoder.*/
	[CRepr]
	public struct LodePNGEncoderSettings {
	  ...
	  c_uint force_palette;
	#if LODEPNG_COMPILE_ANCILLARY_CHUNKS
	  /*add LodePNG identifier and version as a text chunk, for debugging*/
	  c_uint add_id;
	  /*encode text chunks as zTXt chunks instead of tEXt chunks, and use compression in iTXt chunks*/
	  c_uint text_compression;
	#endif /*LODEPNG_COMPILE_ANCILLARY_CHUNKS*/
	}
	
	[CLink] public static extern void lodepng_encoder_settings_init(LodePNGEncoderSettings* settings);
#endif /*LODEPNG_COMPILE_ENCODER*/
    ...
    /*init, cleanup and copy functions to use with this struct*/
    [CLink] public static extern void lodepng_state_init(LodePNGState* state);

    [CLink] public static extern void lodepng_state_cleanup(LodePNGState* state);

    [CLink] public static extern void lodepng_state_copy(LodePNGState* dest, LodePNGState* source);

#endif /* LODEPNG_COMPILE_DECODER || LODEPNG_COMPILE_ENCODER */
    ...
    */
    [CLink] public static extern c_uint lodepng_decode(c_uchar** out_, c_uint* w, c_uint* h, LodePNGState* state, c_uchar* in_, size_t insize);
    ...
    */
    [CLink] public static extern c_uint lodepng_inspect(c_uint* w, c_uint* h, LodePNGState* state, c_uchar* in_, size_t insize);
#endif /*LODEPNG_COMPILE_DECODER*/
...
*/
[CLink] public static extern c_uint lodepng_inspect_chunk(LodePNGState* state, size_t pos, c_uchar* in_, size_t insize);

#if LODEPNG_COMPILE_ENCODER
    /*This function allocates the out buffer with standard malloc and stores the size in *outsize.*/
    [CLink] public static extern c_uint lodepng_encode(c_uchar** out_, size_t* outsize, c_uchar* image, c_uint w, c_uint h, LodePNGState* state);
#endif /*LODEPNG_COMPILE_ENCODER*/
...
*/
[CLink] public static extern c_uint lodepng_chunk_length(c_uchar* chunk);

/*puts the 4-byte type in null terminated string*/
[CLink] public static extern void lodepng_chunk_type(char[5] type, c_uchar* chunk);

/*check if the type is the given type*/
[CLink] public static extern c_uchar lodepng_chunk_type_equals(c_uchar* chunk, char* type);

/*0: it's one of the critical chunk types, 1: it's an ancillary chunk (see PNG standard)*/
[CLink] public static extern c_uchar lodepng_chunk_ancillary(c_uchar* chunk);

/*0: public, 1: private (see PNG standard)*/
[CLink] public static extern c_uchar lodepng_chunk_private(c_uchar* chunk);

/*0: the chunk is unsafe to copy, 1: the chunk is safe to copy (see PNG standard)*/
[CLink] public static extern c_uchar lodepng_chunk_safetocopy(c_uchar* chunk);

/*get pointer to the data of the chunk, where the input points to the header of the chunk*/
[CLink] public static extern c_uchar* lodepng_chunk_data(c_uchar* chunk);
[CLink] public static extern c_uchar* lodepng_chunk_data_const(c_uchar* chunk);

/*returns 0 if the crc is correct, 1 if it's incorrect (0 for OK as usual!)*/
[CLink] public static extern c_uint lodepng_chunk_check_crc(c_uchar* chunk);

/*generates the correct CRC from the data and puts it in the last 4 bytes of the chunk*/
[CLink] public static extern void lodepng_chunk_generate_crc(c_uchar* chunk);
...
*/
[CLink] public static extern c_uchar* lodepng_chunk_next(c_uchar* chunk, c_uchar* end);
[CLink] public static extern c_uchar* lodepng_chunk_next_const(c_uchar* chunk, c_uchar* end);

/*Finds the first chunk with the given type in the range [chunk, end), or returns NULL if not found.*/
[CLink] public static extern c_uchar* lodepng_chunk_find(c_uchar* chunk, c_uchar* end, char[5] type);
[CLink] public static extern c_uchar* lodepng_chunk_find_const(c_uchar* chunk, c_uchar* end, char[5] type);
...
*/
[CLink] public static extern c_uint lodepng_chunk_append(c_uchar** out_, size_t* outsize, c_uchar* chunk);
...
*/
[CLink] public static extern c_uint lodepng_chunk_create(c_uchar** out_, size_t* outsize, size_t length, char* type, c_uchar* data);


/*Calculate CRC32 of buffer*/
[CLink] public static extern c_uint lodepng_crc32(c_uchar* buf, size_t len);
...
*/
    ...
    [CLink] public static extern c_uint lodepng_inflate(c_uchar** out_, size_t* outsize, c_uchar* in_, size_t insize, LodePNGDecompressSettings* settings);
    ...
    */
    [CLink] public static extern c_uint lodepng_zlib_decompress(c_uchar** out_, size_t* outsize, c_uchar* in_, size_t insize, LodePNGDecompressSettings* settings);
#endif /*LODEPNG_COMPILE_DECODER*/
    ...
    buffer and *outsize its size in bytes. out must be freed by user after usage.
    */
    [CLink] public static extern c_uint lodepng_zlib_compress(c_uchar** out_, size_t* outsize, c_uchar* in_, size_t insize, LodePNGCompressSettings* settings);

    /*
    Find length-limited Huffman code for given frequencies. This function is in the
    public interface only for tests, it's used internally by lodepng_deflate.
    */
    [CLink] public static extern c_uint lodepng_huffman_code_lengths(c_uint* lengths, c_uint* frequencies, size_t numcodes, c_uint maxbitlen);

    /*Compress a buffer with deflate. See RFC 1951. Out buffer must be freed after use.*/
    [CLink] public static extern c_uint lodepng_deflate(c_uchar** out_, size_t* outsize, c_uchar* in_, size_t insize, LodePNGCompressSettings* settings);
#endif /*LODEPNG_COMPILE_ENCODER*/
    ...
    to handle such files and decode in-memory.
    */
    [CLink] public static extern c_uint lodepng_load_file(c_uchar** out_, size_t* outsize, char* filename);
    ...
    to handle such files and encode in-memory
    */
    [CLink] public static extern c_uint lodepng_save_file(c_uchar* buffer, size_t buffersize, char* filename);
#endif /*LODEPNG_COMPILE_DISK*/
...
the additional C++ code is in "#if __cplusplus" blocks that make C-compilers
...
format with 4 bytes (c_uchars) per pixel.
...
LodePNG uses c_uchar arrays for 16-bit per channel colors too, just like
...
anyway, but it means you cannot simply cast the c_uchar* buffer to an
c_ushort* buffer on x86 CPUs.
...
void lodepng_chunk_type(char type[5], c_uchar* chunk):
c_uchar lodepng_chunk_type_equals(c_uchar* chunk, char* type):

Get the type of the chunk or compare if it's a certain type

c_uchar lodepng_chunk_critical(c_uchar* chunk):
c_uchar lodepng_chunk_private(c_uchar* chunk):
c_uchar lodepng_chunk_safetocopy(c_uchar* chunk):
...
c_uchar* lodepng_chunk_data(c_uchar* chunk):
c_uchar* lodepng_chunk_data_const(c_uchar* chunk):

Get a pointer to the start of the data of the chunk.

[CLink] public static extern c_uint lodepng_chunk_check_crc(c_uchar* chunk):
void lodepng_chunk_generate_crc(c_uchar* chunk):

Check if the crc is correct or generate a correct one.

c_uchar* lodepng_chunk_next(c_uchar* chunk):
c_uchar* lodepng_chunk_next_const(c_uchar* chunk):
...
[CLink] public static extern c_uint lodepng_chunk_append(c_uchar** out_, size_t* outsize, c_uchar* chunk):
[CLink] public static extern c_uint lodepng_chunk_create(c_uchar** out_, size_t* outsize, c_uint length,
                              char* type, c_uchar* data):
...
CHAR_BITS must be 8 or higher, because LodePNG uses c_uchars for octets.
...
int main(int argc, char *argv[]) {
  ...
  //load and decode
  std::vector<c_uchar> image;
  c_uint width, height;
  c_uint error = lodepng::decode(image, width, height, filename);
...
*) 29 dec 2007: (!) changed most integer types to c_uint int + other tweaks
...
*) 21 jun 2006: (!) Changed type of info vector to c_uint.
    Changed position of palette in info vector. Fixed an important bug that
    happened on PNGs with an uncompressed block.
*) 16 jun 2006: Internally changed c_uint into c_uint where
